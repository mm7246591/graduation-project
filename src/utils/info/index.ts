export const warns=[
    {
        id:"1",
        img:"/src/assets/img/mask.png",
        text:"配戴口罩"        
    },
    {
        id:"2",
        img:"/src/assets/img/smoke.png",
        text:"禁止吸菸"        
    },
    {
        id:"3",
        img:"/src/assets/img/dangerous.png",
        text:"禁止危險物品"        
    },
    {
        id:"4",
        img:"/src/assets/img/trash.png",
        text:"請勿亂丟垃圾"        
    },
    {
        id:"5",
        img:"/src/assets/img/running.png",
        text:"請勿奔跑、喧嘩"        
    }
]

export const videos=[
    {
        id: "1",
        group: "影視組",
        poster: "/src/assets/img/works/film/1.jpg",
        poster_mobile:"/src/assets/img/works/film/1-mobile.jpg",
        imgs: [
            "/src/assets/img/works/film/1.jpg",
            "https://fakeimg.pl/350x200/?text=1",
            "https://fakeimg.pl/350x200/?text=2",
            "https://fakeimg.pl/350x200/?text=3",
            "https://fakeimg.pl/350x200/?text=4"
        ],
        title: "那夜星空下的流星",
        studio: "晨光工作室",
        intro:
        "另一個世界的女孩，因一場意外來到了一個如同末世般的世界，並遇到了一位生活在這的大叔與一隻機器狗。 大叔收容了女孩一起生活，漸漸地，女孩領悟到了家鄉所沒有的情感，學會了快樂與悲傷，也得到了「家」的溫暖。",
        member: "王鈞杭、劉俊廷、莊祥穎、劉智傑、李為璘",
        job:
        "王鈞杭：上色、3D建模、剪輯師" +
        "\n" +
        "劉俊廷：色指定、人物設計、原畫師" +
        "\n" +
        "莊祥穎：導演、動畫師、合成" +
        "\n" +
        "劉智傑：上色、合成、剪輯師" +
        "\n" +
        "李為璘：3D建模、動畫師",
    },
    {
        id: "2",
        group: "影視組",
        poster: "/src/assets/img/works/film/2.jpeg",
        poster_mobile:"/src/assets/img/works/film/2-mobile.jpg",
        imgs: [
            "/src/assets/img/works/film/2.jpeg",
            "https://fakeimg.pl/350x200/?text=1",
            "https://fakeimg.pl/350x200/?text=2",
            "https://fakeimg.pl/350x200/?text=3",
            "https://fakeimg.pl/350x200/?text=4"
        ],
        title: "消失的女兒",
        studio: "MADADAYO STUDIO",
        intro:
        "我們的作品是懸疑類的劇情微電影，從母女之間的情感做發想，發現「控制」是親情中最容易產生且會讓親人之間出現隔閡的行爲。現實生活中的子女年幼時被父母限制人生的走向，成年後又用控制的方式代替照顧，惡性循環。我們塑造了一種極端的「控制」，將藏在家庭關係中隱形的控制放大化、具體化，直觀地讓觀衆體會到控制的感受。",
        member: "黃昱軒、王怡潔、簡文勝、邱翊柏、李美文、陳柏旭",
        job:
        "黃昱軒：燈光" +
        "\n" +
        "王怡潔：美術" +
        "\n" +
        "簡文勝：製片" +
        "\n" +
        "邱翊柏：攝影" +
        "\n" +
        "李美文：導演" +
        "\n" +
        "陳柏旭：收音",
    },
    {
        id: "3",
        group: "影視組",
        poster: "/src/assets/img/works/film/3.png",
        poster_mobile:"/src/assets/img/works/film/3-mobile.jpg",
        imgs: [
            "/src/assets/img/works/film/3.png",
            "https://fakeimg.pl/350x200/?text=1",
            "https://fakeimg.pl/350x200/?text=2",
            "https://fakeimg.pl/350x200/?text=3",
            "https://fakeimg.pl/350x200/?text=4"
        ],
        title: "Titta&Me",
        studio: "玩具工坊",
        intro:
        "一部關於在同一件事物上每個人價值會有所不同,透過主角的故事來看到在對待壞掉的玩具,一般人會覺得壞掉了換新的就好,但在主角的心中就算玩具壞掉不能再玩但是他在主角心中還是一個珍重的寶物不能替代。",
        member: "彭欣慧、陳逸欣、張芝琳、趙婉芸、范皓凌、郭心柔",
        job:
        "彭欣慧：編劇、角色設計、補幀" +
        "\n" +
        "陳逸欣：色彩計畫、角色設計、整合" +
        "\n" +
        "張芝琳：場景設計、分鏡監督、作畫監督" +
        "\n" +
        "趙婉芸：場景設計、分鏡監督" +
        "\n" +
        "范皓凌：角色設計、補幀" +
        "\n" +
        "郭心柔：企劃撰寫、動畫後製",
    },
    {
        id: "4",
        group: "影視組",
        poster: "/src/assets/img/works/film/4.jpg",
        poster_mobile:"/src/assets/img/works/film/4-mobile.jpg",
        imgs: [
            "/src/assets/img/works/film/4.jpg",
            "https://fakeimg.pl/350x200/?text=1",
            "https://fakeimg.pl/350x200/?text=2",
            "https://fakeimg.pl/350x200/?text=3",
            "https://fakeimg.pl/350x200/?text=4"
        ],
        title: "噬",
        studio: "有沒有可能工作室",
        intro:
        "金家代代相傳一本書，書的最後一頁寫著「得越多，失越多。不離不棄，至死方休。」相傳這本書有讓人心想事成的法力，但怪的是卻提醒後代絕對不要打開。而主角金玄豪會如何面對這股神奇魔力呢？",
        member: "許雨雯、曾佳容、郭俊語、方湘楀、陳怡如、郭岱旻",
        job:
        "許雨雯：導演、調色、剪輯" +
        "\n" +
        "曾佳容：攝影、特效、調色" +
        "\n" +
        "郭俊語：美術、妝髮、展場及周邊設計" +
        "\n" +
        "陳怡如：製片、展場及周邊設計" +
        "\n" +
        "方湘楀：收音、調色、海報、社群" +
        "\n" +
        "郭岱旻：燈光、社群、音樂、特效",
    }
]