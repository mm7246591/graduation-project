export const games = [
  {
    id: "1",
    group: "遊戲組",
    poster: "/works/game/1.jpg",
    imgs: [
      "/works/game/1/古嶺奇譚-主視覺 - 菜.png",
      "/works/game/1/古嶺奇譚作品圖1 - 菜.png",
      "/works/game/1/古嶺奇譚作品圖2 - 菜.png",
      "/works/game/1/古嶺奇譚作品圖3 - 菜.png",
      "/works/game/1/古嶺奇譚作品圖4 - 菜.png",
      "/works/game/1/古嶺奇譚作品圖5 - 菜.png",
      "/works/game/1/古嶺奇譚作品圖6 - 菜.png"
    ],
    title: "古嶺奇譚",
    studio: "無為工作室",
    intro:
      "浮華亂世,妖魔鬼怪遍地四起、為禍民間。人們為了免受妖怪侵擾,身為除妖師的主角為大家斬去禍害。在遊戲中,玩家將扮演一名初出茅廬的新手除妖師，過程中透過解謎使劇情推進，運用符咒收服妖怪、為民除害。",
    member: "白紀葳、彭奕綾、蔡易辰、張舒涵、江若仙、裴氏瓊如",
    job:
      "蔡易辰：介面設計、遊戲流程整理" +
      "\n" +
      "張舒涵：劇情對話、音樂音效、影片剪輯、社群" +
      "\n" +
      "彭奕綾：遊戲背景設計、道具與UI畫面繪製" +
      "\n" +
      "白紀葳：角色設計/機關繪製/視覺規劃" +
      "\n" +
      "裴氏瓊如：企劃，特效，程式輔助" +
      "\n" +
      "江若仙：程式規劃、主程式設計",
  },
  {
    id: "2",
    group: "遊戲組",
    poster: "/works/game/2.png",
    imgs: [
      "/works/game/2/主視覺 - 摩摩西.png",
      "/works/game/2/人物設定01_RGB - 摩摩西.png",
      "/works/game/2/人物設定02 - 摩摩西.png",
      "/works/game/2/Castle_RGB - 摩摩西.png",
      "/works/game/2/Forest_RGB - 摩摩西.png",
      "/works/game/2/Title_RGB - 摩摩西.png"
    ],
    title: "血墨契約",
    studio: "Project Ink",
    intro:
      "這是一個橫向卷軸的BossRush遊戲，小女孩Ink一覺醒來，發現進入了童話世界裡。鋼筆化為了利刃，童話中的角色變得極具攻擊性。玩家將與主角一起在童話世界裡，挑戰一個個強大的敵人。",
    member: "張文安、洪順祥、鄭暘、許其彬",
    job:
      "張文安/主美術設計暨角色設計師" +
      "\n" +
      "洪順祥/UI設計暨專案管理師" +
      "\n" +
      "鄭暘/場景設計暨企劃" +
      "\n" +
      "許其彬/主程式暨專案版本管理",
  },
  {
    id: "3",
    group: "遊戲組",
    poster: "/works/game/3.jpg",
    imgs: [
      "/works/game/3/主視覺CMYK - 歐嵐.png",
      "/works/game/3/1 - 歐嵐.png",
      "/works/game/3/2 - 歐嵐.jpg",
      "/works/game/3/3 - 歐嵐.png",
      "/works/game/3/4 - 歐嵐.png"
    ],
    title: "電謝之暝",
    studio: "作息不良工作室",
    intro:
      "在一個狂風暴雨的夜晚，工廠遭雷擊停止了運作，身為警衛的玩家必須進入廠中恢復電力，但周遭的不詳氣氛透漏了這裡似乎有其他的不速之客...... 電謝之暝是款第一人稱解謎遊戲，玩家須在工廠中探索，利用手上的裝置解開謎題，一步步接近工廠背後的秘密。",
    member: "王晨、余博洋、陳忻妤、陳修業、朱王韜",
    job:
      "王晨：企劃、程式、PM" +
      "\n" +
      "余博洋：程式" +
      "\n" +
      "陳忻妤：2D美術" +
      "\n" +
      "陳修業：3D美術" +
      "\n" +
      "朱王韜：企劃、3D美術",
  },
  {
    id: "4",
    group: "遊戲組",
    poster: "/works/game/4.jpg",
    imgs: [
      "/works/game/4/Raise-橫(修改後) - phaoo Chen.jpg",
      "/works/game/4/作品內容1.png",
      "/works/game/4/作品內容2.png",
      "/works/game/4/作品內容3.png",
      "/works/game/4/作品內容4.png"
    ],
    title: "Raise",
    studio: "追影工作室",
    intro:
      "Rasie是一款3D、第三人稱的冒險類遊戲，遊戲元素主要聚焦在地底世界、礦石與光。玩家需要透過與礦石的共鳴互動來獲取能量與光線，以利於協助玩家冒險，並且通過各種層層阻礙。",
    member: "呂晉丞、吳子郇、陳品皓",
    job:
      "陳品皓：3D建模、貼圖、場景設計" +
      "\n" +
      "呂晉丞：企劃、遊戲藍圖設計" +
      "\n" +
      "吳子郇：企劃、骨架、動畫、粒子",
  },
];

export const interacts = [
  {
    id: "1",
    group: "互動組",
    poster: "/works/interact/1.jpg",
    imgs: [
      "/works/interact/1/5.4E+7主視覺_CMYK02.png",
      "/works/interact/1/5.4E+7_01-RGB - Julia Hsieh.png",
      "/works/interact/1/5.4E+7_02-RGB - Julia Hsieh.png",
      "/works/interact/1/5.4E+7_03-RGB - Julia Hsieh.png",
      "/works/interact/1/5.4E+7_04羽毛_RGB - Julia Hsieh.png",
      "/works/interact/1/5.4E+7_05鼻子_RGB - Julia Hsieh.png",
      "/works/interact/1/5.4E+7_06太空垃圾_RGB.png"
    ],
    title: "5.4E+7",
    studio: "TA工作室",
    intro:
      "你有想過火星移民計畫時，過程中會發生什麼事呢？  因此我們利用沉浸式互動投影，讓體驗者在我們創造的環境，根據劇情探索火星的旅程，並且利用獵奇的心理和打破第四面牆的模式讓體驗者有新奇的體驗。",
    member: "謝昀軒、吳宜樺、洪立恒、方子樽、陳柏諺",
    job:
      "謝昀軒：組長&美術" +
      "\n" +
      "吳宜樺：美術" +
      "\n" +
      "洪立恒：程式設計" +
      "\n" +
      "方子樽：程式設計" +
      "\n" +
      "陳柏諺：程式設計",
  },
  {
    id: "2",
    group: "互動組",
    poster: "/works/interact/2.jpg",
    imgs: [
      "/works/interact/2/橫式主視覺rgb - Ya Xin Pan.jpg",
      "/works/interact/2/作品照1.jpg",
      "/works/interact/2/作品照2.jpg",
      "/works/interact/2/作品照3.jpg",
      "/works/interact/2/作品照4.jpg"
    ],
    title: "零魂伴侶",
    studio: "白工作室",
    intro:
      "在一個神秘的神殿裡，據說只要召喚出愛神，並將靈魂交給她，就能夠見到自己的靈魂伴侶...... 我們創造一個虛幻的神殿，讓觀眾進來向愛神許願理想的另外一半，並且得到意想不到的答案！",
    member: "施佩妤、薛涵、潘雅歆、陳涓、蔡坤霖、李曜",
    job:
      "潘雅歆：專案管理 / 平面設計 / 動畫設計" +
      "\n" +
      "施佩妤：視覺美術 / 平面設計 / 動畫設計" +
      "\n" +
      "薛涵：財務管理 / 平面設計 / 動畫設計" +
      "\n" +
      "陳涓：展場設計 / 平面設計 / 動畫設計" +
      "\n" +
      "蔡坤霖：人機裝置設計 / 光線感測程式設計 / Unity程式設計" +
      "\n" +
      "李曜：人機裝置設計 / 光線感測程式設計 / Unity程式設計",
  },
  {
    id: "3",
    group: "互動組",
    poster: "/works/interact/3.jpg",
    imgs: [
      "/works/interact/3/海報CMYK_主視覺 - 吳冠緯.png",
      "/works/interact/3/實體_1 - 吳冠緯.png",
      "/works/interact/3/實體_2 - 吳冠緯.png",
      "/works/interact/3/實體_3 - 吳冠緯.png",
      "/works/interact/3/展場建模1_RGB - 吳冠緯.png",
      "/works/interact/3/社群照1_RGB - 吳冠緯.png",
      "/works/interact/3/社群照2_RGB - 吳冠緯.png"
    ],
    title: "噪域",
    studio: "噪音爆破小組",
    intro:
      "嗨！你有聽過海底下的聲音嗎？  Zoom in 台灣，其實有許多噪音影響四周的海底生物，其中鯨豚類的傷害更是顯著。  透過噪域，以互動展場帶領大家一起了解關於噪音及台灣常見鯨豚類的小知識，達到推廣海底噪音的議題。",
    member: "蒲芳榆、周貝貝、吳冠緯、任力加、温存正、涂紜瑄",
    job:
      "蒲芳榆：動畫製作" +
      "\n" +
      "周貝貝：動畫製作" +
      "\n" +
      "吳冠緯：道具美術" +
      "\n" +
      "任力加：平面形象設計" +
      "\n" +
      "温存正：網站設計" +
      "\n" +
      "涂紜瑄：裝置程式設計",
  },
  {
    id: "4",
    group: "互動組",
    poster: "/works/interact/4.jpg",
    imgs: [
      "/works/interact/4/ARK_ME橫式海報.png",
      "/works/interact/4/場地樣貌.png",
      "/works/interact/4/美人魚的畫.png"
    ],
    title: "ARK _ME",
    studio: "Again studio",
    intro:
      "「ARK_ME」名稱源自於中世紀時期盛行的煉金術alchemy，進入展場後會是一個黑暗的未知空間，一開始玩家是以見習煉金術師的身份，來解救被詛咒的老術士，因為老術士過於熱愛研究神話生物們，導致觸犯到神話生物們底線，以致於遭受到詛咒。身為見習生的我們，利用老術士所留下的筆記，一步一步協助老術士治癒身上的症狀，進以成為優秀的煉金術師。",
    member: "張鈺玟、李宜庭、鄭兆晉、蘇于翔、宋柏勳、葉沛宣",
    job:
      "鄭兆晉：程式撰寫及機關製作" +
      "\n" +
      "宋柏勳：程式撰寫及機關製作" +
      "\n" +
      "張鈺玟：道具製作" +
      "\n" +
      "李宜庭：道具製作" +
      "\n" +
      "蘇于翔：道具製作" +
      "\n" +
      "葉沛宣：道具製作美術設計",
  },
  {
    id: "5",
    group: "互動組",
    poster: "/works/interact/5.jpg",
    imgs: [
      "/works/interact/5.jpg",
      "/works/interact/5/IMG_1138 - Mike Chou.png",
      "/works/interact/5/IMG_1139 - Mike Chou.png",
      "/works/interact/5/IMG_9602 2 - Mike Chou.png",
      "/works/interact/5/IMG_9605 2 - Mike Chou.png",
      "/works/interact/5/IMG_1137 2 - Mike Chou.png"
    ],
    title: "The Echo of Lodestar",
    studio: "Chip Studio",
    intro:
      'Chip Studio，以英文諧音"Cheap"做為團隊名稱之啟發。希望我們藉由有限的預算，去呈現最佳的展覽體驗，透過AR 互動畫展，來傳達學生於大學生活中恐會遇到的各種心理感受，讓觀眾有所共鳴，並能沉浸於其中，也希望觀眾藉由展覽能有自己的感悟。',
    member: "蘇煒筑、邱靖文、周方浩、鄭雨桐、陳俞蓁、王維亭",
    job:
      "蘇煒筑：視覺美術設計、文案撰寫" +
      "\n" +
      "邱靖文：專案管理和統籌、UIXI設計" +
      "\n" +
      "周方浩：動態設計、音樂設計" +
      "\n" +
      "鄭雨桐：動態設計、展場設計" +
      "\n" +
      "陳俞蓁：AR互動設計、實體裝置設計" +
      "\n" +
      "王維亭：網頁程式設計、實體裝置設計",
  },
  {
    id: "6",
    group: "互動組",
    poster: "/works/interact/6.jpg",
    imgs: [
      "/works/interact/6/主視覺橫式 - 陳郁臻.png",
      "/works/interact/6/1920X1080_1_工作區域 1 - 陳郁臻.jpg",
      "/works/interact/6/1920X1080_2_工作區域 1 - 陳郁臻.jpg",
      "/works/interact/6/1920X1080_3_工作區域 1 - 陳郁臻.jpg",
      "/works/interact/6/1920X1080_工作區域 1 - 陳郁臻.jpg",
      "/works/interact/6/1920X1080_工作區域 1 複本 - 陳郁臻.jpg"
    ],
    title: "繫念",
    studio: "角刀牛工作室",
    intro:
      "游勇是一個熱愛裁縫的男孩，他最喜歡的娃娃阿平就是由他親手縫製的。某天游勇收到一封信後就再也沒回來，覺得被拋棄的阿平十分無法諒解，它逐漸變得暴戾，房間內也時常傳出詭異聲響，從此再也沒有人敢靠近那棟房子⋯",
    member: "陳郁臻、羅珮瑄、許靜玟、莊文惠、鄭淳云、黃郁淇",
    job:
      "陳郁臻：主視覺設計、主角設計" +
      "\n" +
      "羅珮瑄：展場設計、總務" +
      "\n" +
      "許靜玟：程式設計、裝置製作" +
      "\n" +
      "莊文惠：公關、美術製作" +
      "\n" +
      "鄭淳云：專案管理、裝置製作" +
      "\n" +
      "黃郁淇：程式設計、裝置製作",
  },
  {
    id: "7",
    group: "互動組",
    poster: "/works/interact/7.jpeg",
    imgs: [
      "/works/interact/7/1E4A345B-2688-4780-81D1-4E5D2B5F9460 - 邱郁琳.png",
      "/works/interact/7/IMG_0615 - 邱郁琳.png",
      "/works/interact/7/IMG_2900.jpg",
      "/works/interact/7/IMG_2908 - 邱郁琳.png",
      "/works/interact/7/IMG_9427.jpg"
    ],
    title: "Jazz Box",
    studio: "2205 studio",
    intro:
      "在方形封閉的空間裡，團隊將打造一個爵士酒吧氛圍的展場，利用酒吧中常見的元素作為互動裝置並結合樂隊演出的投影，展現出爵士樂即興與自由的特性，讓觀展者在JazzBox裡組合出自己的爵士特調！",
    member: "邱郁琳、張耘嘉、何若齊、謝昕倢、林劭謙、徐振瑄",
    job:
      "邱郁琳：主視覺設計、貼文設計、專案管理" +
      "\n" +
      "張耘嘉：文案撰寫、廠商聯繫" +
      "\n" +
      "何若齊：logo設計、展場視覺設計、展間規劃、投影動畫原畫" +
      "\n" +
      "林劭謙：企劃書統整與編排、會計" +
      "\n" +
      "徐振瑄：程式編寫、網頁設計、軟硬體測試" +
      "\n" +
      "謝昕倢：動畫製作",
  },
  {
    id: "8",
    group: "互動組",
    poster: "/works/interact/8.jpg",
    imgs: [
      "/works/interact/8/RGB_橫式 - 途中工作室.png",
      "/works/interact/8/Copy of 作品內容示意圖7.jpg",
      "/works/interact/8/Copy of 作品內容示意圖8.jpg",
      "/works/interact/8/Copy of 作品內容示意圖9.jpg",
      "/works/interact/8/作品內容示意圖2 - 途中工作室.jpg"
    ],
    title: "用心眼視",
    studio: "途中工作室",
    intro:
      "由於部分民眾並不太了解視障者，因此容易以不友善的態度對待他們，我們希望能透過VR體驗讓體驗者體會視障者在不同情境中所遭遇到的困難，除了能讓大眾體會其心境外，同時希望體驗結束後能帶給體驗者一些反思。",
    member: "蔡尚恩、陳堂宇、蔡容安、李汶儒、劉文業",
    job:
      "蔡尚恩：美術設計、場景建模、影片剪輯、文字整理" +
      "\n" +
      "蔡容安：美術設計、展場規劃、Unity程式、互動留言牆、粉專經營" +
      "\n" +
      "陳堂宇：腳本規劃、Unity程式、文字整理、文案撰寫" +
      "\n" +
      "李汶儒：美術設計、展場規劃、Unity程式、互動留言牆、粉專經營" +
      "\n" +
      "劉文業：Unity程式、VR裝置與Unity串接、VR場景擺設",
  },
  {
    id: "9",
    group: "互動組",
    poster: "/works/interact/9.jpg",
    imgs: [
      "/works/interact/9/主 - 陳慧心.png",
      "/works/interact/9/1 - 陳慧心.png",
      "/works/interact/9/2 - 陳慧心.png",
      "/works/interact/9/3 - 陳慧心.png",
      "/works/interact/9/4 - 陳慧心.png"
    ],
    title: "不存在的歷史1963",
    studio: "週休二四工作室",
    intro:
      "透過MR混合實境（unity,arduino結合）的方法做出一項混合實境的互動解謎遊戲，展場空間富有美國1960年代感的設計，同時搭配富有臨場感的影音製作，達到完美呈現沈浸式互動解謎的體驗。",
    member: "陳慧心、陳怡安、陳主祥、魏承恩、呂柏龍、林若芸",
    job:
      "陳慧心：展場美術設計/展場布置" +
      "\n" +
      "陳怡安：3D建模/展場設計" +
      "\n" +
      "陳主祥：影視製作/系統設計" +
      "\n" +
      "魏承恩：Arduino程式設計/展場佈線" +
      "\n" +
      "呂柏龍：Hololens開發/系統串接/展場布置" +
      "\n" +
      "林若芸：影視製作/展場設計布置",
  },
  {
    id: "10",
    group: "互動組",
    poster: "/works/interact/10.png",
    imgs: [
      "/works/interact/10/主視覺_cmyk - 家儀.png",
      "/works/interact/10/作品內容-1 - 家儀.png",
      "/works/interact/10/作品內容-2 - 家儀.png",
      "/works/interact/10/作品內容-3 - 家儀.jpg",
      "/works/interact/10/作品內容-4 - 家儀.jpg",
      "/works/interact/10/作品內容-5 - 家儀.png"
    ],
    title: "蒔夢",
    studio: "遲到寶工作室",
    intro:
      "蒔夢是一款以花朵為主軸的密室逃脫遊戲，透過花朵特有的花香與花語進行謎題的設計。 故事主角俊維是一位機械工程師，他厭倦了日復一日的生活，卻不敢改變。直到某天因緣際會下，他的房間出現了變化⋯。",
    member: "洪幸宇、宋翊彰、廖子硯、洪佳妤、李家儀、張亞芸",
    job:
      "洪幸宇：機關設計、展場建模、展場假設" +
      "\n" +
      "宋翊彰：機關設計、Arduino、展場假設" +
      "\n" +
      "廖子硯：影片拍攝、展場假設" +
      "\n" +
      "洪佳妤：美術設計、動畫" +
      "\n" +
      "李家儀：時程規劃、文案構想、組長" +
      "\n" +
      "張亞芸：Unity、美術設計、動畫補間",
  },
  {
    id: "11",
    group: "互動組",
    poster: "/works/interact/11.jpg",
    imgs: [
      "/works/interact/11/混鏡主視覺 - Mina.png",
      "/works/interact/11/混鏡-1 - Mina.jpg",
      "/works/interact/11/混鏡-2 - Mina.jpg",
      "/works/interact/11/混鏡-3 - Mina.jpg",
      "/works/interact/11/混鏡-4 - Mina.jpg"
    ],
    title: "混鏡",
    studio: "小熊軟糖工作室",
    intro:
      "位處郊區的冶世診所今夜在一聲巨響中打破了以往的安寧，唐琥祿醫師被發現陳屍在診所內的樓梯間，他是如何被殺的，又是誰對他痛下毒手······ 兇手是否就在你身邊？和夥伴一起來體驗解謎的樂趣吧！",
    member: "游又潔、吳采芊、都芊如、汪承緒、杜孟洋",
    job:
      "游又潔：插畫設計、繪畫設計、包裝設計" +
      "\n" +
      "吳采芊：平面設計、影片編輯、包裝設計" +
      "\n" +
      "都芊如：平面設計、模型建設、包裝設計" +
      "\n" +
      "汪承緒：程式代碼編輯、場地架設" +
      "\n" +
      "杜孟洋：程式代碼編輯、場地架設",
  },
];

export const markets = [
  {
    id: "1",
    group: "行銷組",
    poster: "/works/market/1.jpg",
    imgs: [
      "/works/market/毛起來＿主視覺立牌（CMYK） - 遠東山口組.png",
      "/works/market/毛起來＿作品＿1 - 遠東山口組.png",
      "/works/market/毛起來＿作品＿2 - 遠東山口組.png",
      "/works/market/毛起來＿作品＿4 - 遠東山口組.JPG",
      "/works/market/毛起來＿作品＿6 - 遠東山口組.jpg",
      "/works/market/志工.png"
    ],
    title: "毛起來",
    studio: "遠東山口工作室",
    intro:
      "品種貓、混種貓、家貓、流浪貓……不管是什麼貓，都不應該有高低貴賤之別，每一隻毛小孩都值得被愛！為了拯救那些渴望被愛的貓咪們，我們打造一套專屬於「小松樹流浪貓咪中途之家」的行銷企劃，致力於改善人力資源不足以及認養率低的問題～  一起加入小松樹流浪貓咪中途之家，毛起來盡一份心力吧！！！",
    member: "詹采融、蔡昀芬、郭璦菁、李健暐、張佳銘、陳栗芩",
    job:
      "蔡昀芬：平面設計、UI介面設計、動畫製作" +
      "\n" +
      "詹采融：對外接洽、影片剪輯、社群經營、文案設計" +
      "\n" +
      "郭璦菁：前端網頁程式設計、Ar製作" +
      "\n" +
      "李建暐：平面設計、社群經營、文案設計、動畫製作" +
      "\n" +
      "張佳明：平面設計、動畫製作" +
      "\n" +
      "陳栗芩：專案管理、企劃發想、影像製作",
  },
];

export const researches = [
  {
    id: "1",
    group: "大專生研究計畫",
    poster: "/works/research/1.png",
    imgs: [
      "/works/research/1/橫式主視覺_CMYK元宇宙平台對使用者自創內容設計與實踐之分析  - 孫睦桓.png",
      "/works/research/1/作品內容 - 孫睦桓.png",
      "/works/research/1/My Sandbox on 23-3-28 at 凌晨1212.png"
    ],
    title: "元宇宙平台對使用者自創內容設計與實踐之分析",
    studio: "無",
    intro:
      "我們將Spatial視為一個開放的元宇宙世界，由使用者自創內容組合而成。本研究藉由訪談探究設計者如何設計元宇宙空間及如何在其中實現自己的想法與創新，並探討使用者對於虛擬空間的感受與體驗，從中發現可能的問題和方向。",
    member: "孫睦桓",
    job:
      "孫睦桓：研究者",
  },
  {
    id: "2",
    group: "大專生研究計畫",
    poster: "/works/research/2.jpg",
    imgs: [
      "/works/research/2/330887913_959134555468467_4028990915777812729_n - 魏承恩.jpg",
      "/works/research/2/mock1.png",
      "/works/research/2/mock2.png"
    ],
    title: "湖口老街AR指標系統",
    studio: "無",
    intro:
      "介紹湖口老街的虛擬實境地圖讓原本的老街注入新風貌，以AR的表現形式介紹該地相關地標，希望藉此能讓使用者利用與以往不同的方式更進一步的深入了解有關湖口老街的相關歷史由來。",
    member: "雷子誼",
    job:
      "雷子誼：撰寫企劃/製作企劃",
  },
];
