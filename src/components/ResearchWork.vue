...
const getData = () => {
    items.value = researches.filter(research => {
        if (research.id === route.params.id) {
            return research
        }
    })
};
        ...
        <div v-for="group of items" :key="group.id">
            ...
            <div class="lg:flex">
                <div class="mx-5 lg:w-1/3 lg:mx-24 text-[#2A3752]">
                    <div
                        class="relative mb-8 lg:flex lg:items-end lg:mb-10 lg:pt-12 font-bold text-2xl before:content-[''] before:absolute before:bottom-0 before:-left-4 lg:before:-left-8 before:w-1.5 lg:before:w-3.5 before:h-full before:rounded-full before:bg-[#00E4FF]">
                        {{ group.title }}</div>
